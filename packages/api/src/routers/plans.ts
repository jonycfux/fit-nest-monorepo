...
export const plansRouter = router({
  // Includes workoutCount for the Dashboard's "N weeks · N workouts" plan card
  // subtext (batch-counted, not per-row).
  list: protectedProcedure.query(async ({ ctx }) => {
    ...
    const counts = planIds.length
      ? await ctx.db
          .select({ planId: planWorkouts.planId, count: sql<number>`count(*)::int` })
          .from(planWorkouts)
    ...
    return plans.map((plan) => ({
      ...plan,
      workoutCount: countByPlan.get(plan.id) ?? 0,
    }));
  }),
  ...
  byId: protectedProcedure.input(idInput).query(async ({ ctx, input }) => {
    const [plan] = await ctx.db
      .select()
      .from(fitnessPlans)
      .where(and(eq(fitnessPlans.id, input.id), eq(fitnessPlans.userId, ctx.user.id)));
    ...
    .mutation(({ ctx, input }) =>
      ctx.db.transaction(async (tx) => {
        const [plan] = await tx
          ...
          .from(fitnessPlans)
          .where(and(eq(fitnessPlans.id, input.planId), eq(fitnessPlans.userId, ctx.user.id)));
        if (!plan) throw new TRPCError({ code: "NOT_FOUND" });
        ...
        if (unique.length > 0) {
          const owned = await tx
            ...
            .from(workouts)
            .where(and(eq(workouts.userId, ctx.user.id), inArray(workouts.id, unique)));
        ...
        await tx.delete(planWorkouts).where(eq(planWorkouts.planId, input.planId));
